...
from Solution.GraphicsView_App import RigidLink
from Solution.GraphicsView_App import RigidPivotPoint
class Position():
    """
...
class TrussView():
    ...
    def buildScene(self, truss=None):
        rect = qtc.QRect()
        rect.setTop(int(truss.nodes[0].position.y))  # Cast to int
        rect.setLeft(int(truss.nodes[0].position.x))  # Cast to int
        rect.setHeight(0)
        rect.setWidth(0)
        for n in truss.nodes:
            if n.position.y > rect.top(): rect.setTop(int(n.position.y))  # Cast to int
            if n.position.y < rect.bottom(): rect.setBottom(int(n.position.y))  # Cast to int
            if n.position.x > rect.right(): rect.setRight(int(n.position.x))  # Cast to int
            if n.position.x < rect.left(): rect.setLeft(int(n.position.x))  # Cast to int
    ...
    def drawLinks(self, truss=None):
        ...
        for l in truss.links:
            ...
            # Create a RigidLink object with the appropriate parameters
            rigid_link = RigidLink(x1, -y1, x2, -y2, pen=qtg.QPen(qtg.QColor("orange")), brush=qtg.QBrush(qtg.QColor.fromHsv(87, 98, 245, 128)))

            # Optionally, set tooltip and other properties if needed
            st = 'link: ' + l.name + '\n'
            st += 'length = {:0.2f}\n'.format(l.length)
            st += 'angle deg = {:0.2f}'.format(l.angleRad * 180.0 / math.pi)
            rigid_link.setToolTip(st)

            # Add the RigidLink to the scene
            scene.addItem(rigid_link)
    ...
    def drawNodes(self, truss=None, scene=None):
        ...
        for n in truss.nodes:
            ...
            # Only apply RigidPivotPoint to left and right nodes
            if n.name.lower() in ['left', 'right']:
                rigid_pivot_point = RigidPivotPoint(x, y, pivotHeight=10, pivotWidth=23, pen=qtg.QPen(qtc.Qt.black))

                scene.addItem(rigid_pivot_point)
            else:
